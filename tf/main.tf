...
locals {
  ...
  ingestion_glue_job_file = "default_glue_ingestion_job.py"
...
data "template_file" "state_machine_definition" {
  ...
  vars = {
    ...
    destination_path_prefix = var.destination_path_prefix
...
resource "aws_security_group" "phoenix_raw_data_to_parquet_security_group" {
...
resource "aws_s3_bucket_object" "basic_glue_job" {
  ...
  key    = "${local.s3_scripts_path}/${local.ingestion_glue_job_file}"
  source = local.ingestion_glue_job_file
  etag   = filemd5(local.ingestion_glue_job_file)
}
# ---------------------------------------------------------------------------------------------------------------------
